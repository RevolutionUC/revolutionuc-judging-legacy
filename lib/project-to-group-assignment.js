...
module.exports = (projects, config) => {
    ...
    let reverseAfterRotation = false
    let toBeReversedAfter = 0
        ...
        submissions.forEach(project => {
            if (groups[groupIndex].projects.length >= projectsPerGroup) {
                if (reverseAfterRotation && ((i % 2) === 0)) {
                    groups[groupIndex].projects = groups[groupIndex].projects.sort()
                } else {
                    groups[groupIndex].projects =  groups[groupIndex].projects.reverse()
                }
                groupIndex++
            }
    ...
    return groups.map(group => {
        group.projects = group.projects.map(title => projects[title])
        return group
    })
}
